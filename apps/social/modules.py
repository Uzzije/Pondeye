...
def recent_upload_comments_jsonified(recent_upload, timezone='UTC'):
    comments = CommentRecentUploads.objects.filter(recent_upload=recent_upload, is_deleted=False)
    comments_list = []
    for comm in comments:
        com_dic = {
            'first_name':comm.tikedge_user.user.first_name,
...
def get_grade(user):
    ...
    if projects.count() > 0:
        grade = ((projects.count() - failed.count())/projects.count()) * 100
        if grade > 80:
            return 'A'
        elif grade > 70:
            return 'B'
...
def make_timeline_video(progress_set):
    ...
    for each_prog in progress_set.list_of_progress_videos.all():
        ...
        txt_clip = txt_clip.set_pos(('left','bottom')).set_duration(file_.duration)
