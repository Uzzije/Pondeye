...
class ApiNewChallenge(CSRFExemptView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if time_has_past(end_by, timezone=timezone):
            ...
            return HttpResponse(json.dumps(response), status=201)
        tikedge_user = TikedgeUser.objects.get(user=user)
        challenge_id = int(request.POST.get('challenged_user'))
        if challenge_id > 0:
        ...
        new_challenge = Challenge(project=new_project, challenged=challenged_user, challenge_accepted=accepted,
                                       date_responded=dt_res, challenge_responded=challenge_responded,
...
class ApiProfileView(CSRFExemptView):
    ...
    def get(self, request):
        ...
        friend_count = len(Friend.objects.friends(other_user))
